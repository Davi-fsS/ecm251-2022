...
class Application:
    ...
    def visualizar_pedido(self):
        ...
        itens_pedido = self.pedido_controller.pegar_pedido(numero_pedido = self.cliente_pedido_atual.id_pedido) 
        for item in itens_pedido:
            item_data = self.item_controller.pegar_item(id = item.id_item)
            retorno["itens"].append({"nome":item_data.nome,"preco":item_data.preco,"quantidade":item.quantidade})
        return retorno
    
    def fechar_pedido(self):
        return self.pedido_controller.total_pedido(numero_pedido = self.cliente_pedido_atual.id_pedido)